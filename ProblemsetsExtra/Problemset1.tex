...
Escriba un script en Python que defina el espacio muestral $S$ como un conjunto de cadenas de texto, y utilice la biblioteca itertools para generar y mostrar todas las secuencias posibles de clasificaciones para un cúmulo de $N=3$ galaxias.
...
\item Genere un histograma con las frecuencias absolutas del número de noches necesarias para lograr el éxito en las 1000 simulaciones, asegurándose de etiquetar correctamente los ejes y el título de la figura utilizando la biblioteca matplotlib.
\end{enumerate}
...
\item Utilice la biblioteca matplotlib para crear un gráfico de dispersión de las coordenadas generadas, proyectadas en un sistema donde el eje horizontal represente la Ascensión Recta en horas y el eje vertical la Declinación en grados.
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que defina el espacio muestral $S$ como una lista de cadenas de texto o tuplas. Utilice operaciones de conjuntos nativas de Python para filtrar e imprimir en pantalla los elementos que conforman los eventos $A$ y $B$.
\item Simule la detección de $N = 1000$ pares de asteroides. Asuma que las probabilidades marginales de encontrar un asteroide de tipo C, S y X son $P(C) = 0.7$, $P(S) = 0.2$ y $P(X) = 0.1$, respectivamente, y que las clasificaciones de ambos asteroides son eventos independientes.
\item A partir de los datos generados en la simulación, calcule la probabilidad empírica de que ocurra el evento $A$ y compárela con el valor de probabilidad teórica calculada analíticamente.
\item Genere un gráfico de barras que muestre la frecuencia absoluta de cada uno de los 9 eventos simples del espacio muestral $S$ obtenidos en la simulación, asegurándose de etiquetar correctamente los ejes y el título utilizando la biblioteca matplotlib.
\end{enumerate}
...
\section{Demostración de la Probabilidad del Evento Nulo mediante Axiomas}

En el marco de los fundamentos axiomáticos de la probabilidad, el tercer axioma establece la aditividad numerable para sucesiones de eventos mutuamente excluyentes. Sea $(\Omega, \mathcal{F}, P)$ un espacio de probabilidad. Considere una sucesión infinita de eventos $A_1, A_2, A_3, \dots$ definidos en el espacio muestral, donde cada evento es idénticamente el conjunto vacío, es decir, $A_i = \emptyset$ para todo $i \geq 1$.

\begin{enumerate}[a)]
\item Demuestre que la sucesión de eventos $A_i$ cumple con la condición de ser mutuamente excluyentes, calculando la intersección de cualquier par de eventos $A_i \cap A_j$ para $i \neq j$.
\item Determine el conjunto resultante de la unión infinita de estos eventos, expresado formalmente como $\bigcup_{i=1}^{\infty} A_i$.
\item Aplicando rigurosamente el tercer axioma de la probabilidad, plantee la ecuación que relaciona la probabilidad de la unión infinita con la suma infinita de las probabilidades individuales. La expresión matemática base a evaluar es:
$$
P\left(\bigcup_{i=1}^{\infty} A_i\right) = \sum_{i=1}^{\infty} P(A_i)
$$
\item A partir de la ecuación obtenida en el inciso anterior, deduzca analíticamente el valor exacto de la probabilidad del evento nulo, $P(\emptyset)$
\end{enumerate}

\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Implemente una simulación numérica que calcule la suma parcial $S_N = \sum_{i=1}^{N} p$ para un valor de prueba $p = 0.05$. Grafique el comportamiento de $S_N$ en función de $N$ para $N$ desde 1 hasta 1000 utilizando matplotlib, demostrando visualmente la divergencia de la serie si $p > 0$.
\item Modifique el script anterior para evaluar la suma parcial con $p = 0$. Genere una tabla comparativa en la consola que muestre los valores de $S_N$ para $N = 10, 100, 1000$ en ambos casos ($p = 0.05$ y $p = 0$), evidenciando numéricamente la necesidad lógica de que la probabilidad del evento nulo sea estrictamente cero para no violar los axiomas de probabilidad.
\end{enumerate}
...
\begin{enumerate}[a)]
\item Verifique analíticamente que las probabilidades asignadas satisfacen los axiomas de probabilidad para el espacio muestral $S$, demostrando que se cumple la condición de normalización:
$$ \sum_{i \in \{A, B, C\}} P(i) = 1 $$
\item Calcule la probabilidad de que una nueva señal candidata sea de origen astrofísico genuino (es decir, que ocurra el evento $A \cup B$).
...
\item Si se define un nuevo evento $D$ como la ocurrencia de una fusión de agujeros negros o un falso positivo ($D = A \cup C$), calcule $P(D)$ y demuestre que $P(D) = 1 - P(B)$ utilizando las propiedades fundamentales derivadas de los axiomas de probabilidad.
\end{enumerate}

\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Simule la detección de $N = 10000$ señales candidatas utilizando las probabilidades dadas. Utilice la función de elección aleatoria ponderada de la biblioteca numpy para generar las clasificaciones de cada evento.
\item A partir de los datos simulados, calcule las frecuencias relativas de los eventos $A$, $B$ y $C$. Compare estos valores empíricos con las probabilidades teóricas y genere un gráfico de barras agrupadas que muestre ambas comparaciones, asegurándose de incluir leyendas, etiquetas en los ejes y un título claro utilizando matplotlib.
\item Calcule empíricamente la probabilidad de que una señal sea de origen astrofísico ($A \cup B$) a partir de la simulación y verifique que el resultado coincide con el valor teórico calculado en el inciso b.
\end{enumerate}
...
\item Calcule la probabilidad de que el rover complete el ciclo de perforación de manera exitosa, es decir, $P(F')$
\item Para garantizar los objetivos científicos de la misión, se requiere que la probabilidad acumulada de obtener al menos una muestra geológica exitosa sea superior al 99\%. Asumiendo que los ciclos operativos son independientes entre sí, determine analíticamente el número mínimo de ciclos redundantes $n$ que deben programarse.
\end{enumerate}

\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que simule $N = 10000$ ciclos operativos del rover. Utilice la biblioteca numpy para generar los resultados de cada ciclo asignando las categorías $A$, $B$, $C$ y $F'$ según sus probabilidades teóricas.
\item A partir de los datos simulados, calcule las frecuencias relativas de cada modo de falla y del evento de éxito $F'$. Genere un gráfico de barras que compare las probabilidades empíricas con las teóricas, asegurándose de incluir leyendas y etiquetas claras utilizando matplotlib.
\item Implemente una función que calcule la probabilidad acumulada de éxito en función del número de ciclos programados $n$. Genere una gráfica de esta función para $n$ desde 1 hasta 200, y marque visualmente el punto exacto donde la probabilidad acumulada supera el umbral del 99\%.
\end{enumerate}
...
\begin{enumerate}[a)]
\item Calcule la probabilidad total de que el radiotelescopio registre un albedo radar alto, es decir, determine el valor de $P(A)$ utilizando la Ley de Probabilidad Total.
\item Si una nueva observación arroja un albedo radar alto (evento $A$), calcule la probabilidad de que el asteroide sea realmente de tipo metálico ($M$). 
\item Defina el evento $B$ como la medición de un albedo radar bajo o moderado ($\hat{\rho} \leq 0.3$). Determine la probabilidad condicional $P(S|B)$ y discuta cómo este resultado afecta la confianza en la clasificación de asteroides rocosos cuando la señal de retorno es débil.
\end{enumerate}
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que simule el paso de $N = 100,000$ candidatos a señal a través de las tres etapas del pipeline original, utilizando la biblioteca numpy para generar los resultados secuenciales basados en las probabilidades condicionales dadas.
...
\item Genere un gráfico de barras de embudo utilizando matplotlib que muestre el número de señales que sobreviven en cada etapa, ilustrando visualmente la pérdida de candidatos en el pipeline.
\item Realice un análisis de sensibilidad: cree una malla de valores para $P(E_2 | E_1)$ y $P(E_3 | E_1 \cap E_2)$ variando de 0.1 a 0.99. Calcule la probabilidad final para cada combinación y genere un mapa de calor con matplotlib para visualizar qué etapa condicional impacta más drásticamente la eficiencia global del sistema.
\end{enumerate}
...
\item Para los eventos mutuamente excluyentes $M_1$ y $M_2$, calcule $P(M_1 \cup M_2)$ y $P(M_1 \cap M_2)$. Explique analíticamente por qué la probabilidad de su intersección es cero y cómo esto contrasta con la intersección de los subsistemas independientes $A$, $B$ y $C$.
\end{enumerate}

\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que utilice la biblioteca numpy para simular el funcionamiento de los tres subsistemas críticos durante $N = 100,000$ periodos de un año. Genere matrices aleatorias donde cada columna represente un subsistema y cada fila un periodo, asignando 1 (éxito) o 0 (fallo) según sus respectivas probabilidades.
...
\item Genere un gráfico de barras con matplotlib que muestre las probabilidades teóricas de las 8 combinaciones posibles de éxito y fallo de los subsistemas $A$, $B$ y $C$ (por ejemplo, el estado donde todos fallan, donde solo A falla, etc.). Asegúrese de etiquetar los ejes y el título correctamente.
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que utilice la biblioteca numpy para simular $10^7$ intervalos de 1 milisegundo. Genere arreglos booleanos para $N_1$, $N_2$ y $W_{ok}$ basándose en sus probabilidades teóricas, y calcule las frecuencias empíricas de los eventos $C$ y $A$.
...
\begin{enumerate}[a)]
\item Calcule la probabilidad total de que el espectrómetro detecte la banda de absorción en 1 µm, aplicando la Ley de Probabilidad Total.
\item Utilice el Teorema de Bayes para determinar las probabilidades a posteriori de cada hipótesis. La expresión matemática general para este cálculo es:
$$
...
\item Suponga que el rover realiza una segunda medición $C$ (una banda de absorción en 1.05 µm asociada a olivino). Plantee la expresión analítica para la actualización secuencial bayesiana $P(A_1|B \cap C)$, asumiendo que las mediciones son condicionalmente independientes dada la composición real de la roca.
\end{enumerate}

\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que simule el análisis de $N = 100,000$ afloramientos rocosos. Utilice la biblioteca numpy para generar las composiciones reales según las probabilidades a priori y, posteriormente, simule la detección del evento $B$ usando las verosimilitudes dadas. Calcule las frecuencias relativas empíricas a posteriori y compárelas con los valores teóricos.
\item Genere un diagrama de Sankey utilizando la biblioteca plotly para visualizar el flujo de las 100,000 simulaciones. El diagrama debe mostrar cómo las rocas de cada tipo real (nodo izquierdo) se distribuyen entre la detección y la no detección de la banda de 1 µm (nodo derecho), permitiendo identificar visualmente la tasa de falsos positivos para las condritas carbonáceas.
\item Implemente un análisis de sensibilidad mediante un bucle en Python. Varíe la probabilidad a priori $P(A_1)$ desde 0.01 hasta 0.99 (distribuyendo el resto equitativamente entre $A_2$ y $A_3$). Para cada valor, calcule la probabilidad a posteriori $P(A_1|B)$ y genere una gráfica con matplotlib que muestre cómo la evidencia espectral $B$ logra dominar sobre la creencia a priori, identificando el umbral exacto donde la probabilidad a posteriori supera el 95\%.
\end{enumerate}
...
\begin{enumerate}[a)]
\item Calcule la probabilidad total de obtener una medición $\hat{e} \geq 1.0$, aplicando rigurosamente la Ley de Probabilidad Total.
\item Utilice el Teorema de Bayes para determinar la probabilidad a posteriori de que el cometa sea realmente interestelar ($H_H$) dado que la medición arrojó $\hat{e} \geq 1.0$. La expresión matemática para este cálculo es:
$$
P(H_H|M) = \frac{P(M|H_H)P(H_H)}{P(M|H_H)P(H_H) + P(M|H_P)P(H_P)}
$$
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que simule el descubrimiento de $N = 50,000$ cometas. Utilice la biblioteca numpy para asignar la naturaleza real de cada cometa ($H_P$ o $H_H$) según las probabilidades a priori, y luego simule el resultado de la medición ($M$ o su complemento) usando las verosimilitudes dadas. Calcule la probabilidad empírica de que un cometa sea $H_H$ dado que ocurrió el evento $M$, y compárela con el valor teórico calculado en el inciso b.
\item Genere una matriz de confusión visual utilizando la biblioteca seaborn o matplotlib. La matriz debe mostrar la distribución de los 50,000 cometas simulados, con la verdadera naturaleza del cometa en el eje vertical y el resultado de la medición en el eje horizontal. Añada anotaciones numéricas en cada celda para resaltar la cantidad de falsos positivos frente a las detecciones correctas, permitiendo evaluar visualmente el rendimiento del telescopio.
\item Realice un análisis de sensibilidad para evaluar el impacto de la precisión instrumental en la clasificación. Varíe la tasa de falsos positivos $P(M|H_P)$ desde $0.01$ hasta $0.50$ en pasos de $0.01$, manteniendo constantes el resto de los parámetros. Para cada valor, calcule la probabilidad a posteriori $P(H_H|M)$ y genere una gráfica con matplotlib que muestre esta evolución. Determine e indique visualmente en la gráfica el umbral exacto de precisión (el valor de $P(M|H_P)$) en el cual la probabilidad a posteriori de que el cometa sea interestelar supera el 50\%.
\end{enumerate}
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que utilice la biblioteca itertools para generar el espacio muestral completo de configuraciones como un producto cartesiano y verifique el conteo total comparándolo con el resultado analítico.
\item Implemente una función que filtre las configuraciones generadas según la restricción del inciso b y cuente los casos válidos, imprimiendo en consola la proporción de configuraciones que soportan alta fidelidad respecto al total.
\item Simule la selección aleatoria de $10^5$ configuraciones utilizando numpy. A partir de esta muestra, grafique un diagrama de barras con la frecuencia de aparición de cada uno de los 6 filtros, validando empíricamente la probabilidad teórica del inciso c.
\end{enumerate}
...
\begin{enumerate}[a)]
\item Calcule el número total de secuencias de observación distintas que puede generar el telescopio, utilizando la fórmula de permutaciones. La expresión matemática que rige este cálculo es:
$$ P(n, k) = \frac{n!}{(n-k)!} $$
...
\item Genere todas las permutaciones posibles de 5 elementos elegidos de un conjunto de 12 utilizando itertools. Aplique un filtro programático para contar cuántas secuencias cumplen la restricción de no consecutividad del inciso c y compare este resultado exhaustivo con el cálculo analítico.
...
\begin{enumerate}[a)]
\item Calcule el número total de secuencias de observación distintas que puede programar el telescopio, utilizando la fórmula de permutaciones sin repetición:
$$ P(n, k) = \frac{n!}{(n-k)!} $$
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que utilice la biblioteca math para calcular los valores teóricos de los incisos a, b y c. Imprima los resultados en consola con un formato claro y estructurado.
\item Dado que el espacio muestral total es grande, reduzca el problema a $n = 6$ objetivos y $k = 3$ observaciones. Utilice la biblioteca itertools para generar exhaustivamente todas las permutaciones posibles. Filtre programáticamente aquellas que cumplan la restricción del inciso b (los dos objetivos específicos no son consecutivos) y compare el conteo resultante con la fórmula analítica adaptada a estos nuevos valores.
\item Genere una matriz de transición (heatmap) utilizando matplotlib y seaborn. La matriz debe mostrar la frecuencia con la que cada objetivo $i$ es observado inmediatamente después del objetivo $j$ en un conjunto de 10,000 secuencias aleatorias válidas (generadas con numpy), permitiendo visualizar visualmente las restricciones de adyacencia térmica impuestas.
\end{enumerate}
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que defina una función recursiva o utilice itertools.permutations para generar todas las secuencias posibles para un caso reducido de $n = 5$ copas y $k = 3$ análisis. Cuente e imprima cuántas de estas secuencias cumplen la restricción del inciso c (la copa $V$ está en la posición central).
\item Simule la ejecución de 50,000 secuencias de análisis aleatorias para el caso original ($n = 12, k = 5$). Utilice un arreglo de numpy para almacenar la posición en la que aparece la muestra de alta prioridad $A$ en cada simulación. Genere un gráfico de barras con matplotlib que muestre la distribución de frecuencias de la posición de la muestra $A$, verificando empíricamente la uniformidad de las permutaciones.
\item Implemente un algoritmo de backtracking en Python que construya las secuencias paso a paso para el caso $n = 6, k = 4$, asegurando que la copa $V$ siempre caiga en la posición 2 o 3. Utilice la biblioteca networkx para graficar una red que muestre las transiciones desde la primera hasta la última posición de la secuencia, resaltando los nodos donde se inserta la copa $V$.
\end{enumerate}
...
\begin{enumerate}[a)]
\item Calcule el número total de combinaciones posibles de 4 sitios de aterrizaje seleccionados de los 14 candidatos, utilizando la fórmula de combinaciones sin repetición:
$$
\binom{n}{k} = \frac{n!}{k!(n-k)!}
$$
\item Si el comité exige que al menos uno de los 4 sitios seleccionados pertenezca a la zona de criovolcanismo activo (3 regiones disponibles), calcule analíticamente el número de combinaciones válidas utilizando el método de conteo complementario.
\item Suponga ahora que se impone una restricción adicional: la selección debe incluir exactamente 2 sitios de hielo caótico, 1 sitio de crestas lineales y 1 sitio de criovolcanismo. Determine el número de combinaciones que satisfacen esta distribución geológica específica, aplicando el principio multiplicativo a las combinaciones parciales de cada zona.
...
\item Realice una simulación de Monte Carlo con $10^5$ selecciones aleatorias de 4 sitios. Calcule la frecuencia empírica de la condición del inciso d (al menos un sitio de cada zona) y compárela con la probabilidad teórica. Genere un gráfico de convergencia utilizando matplotlib que muestre cómo la probabilidad empírica se estabiliza a medida que aumenta el número de simulaciones desde $10^2$ hasta $10^5$, con el eje x en escala logarítmica.
\end{enumerate}
...
\begin{enumerate}[a)]
\item Calcule el número total de sub-arrays distintos de 6 antenas que se pueden configurar a partir de las 20 disponibles, aplicando la fórmula de combinaciones:
$$
\binom{n}{k} = \frac{n!}{k!(n-k)!}
$$
...
\textbf{Parte computacional}
\begin{enumerate}[a)]
\item Escriba un script en Python que calcule y almacene en un arreglo el número de sub-arrays posibles $\binom{20}{k}$ para cada valor de $k$ desde 1 hasta 20, utilizando la función math.comb. Genere un gráfico de barras con matplotlib que muestre la distribución completa, resaltando en color diferente la barra correspondiente al valor de $k$ que maximiza el número de configuraciones.
\item Para el caso $k = 6$, simule la generación aleatoria de $50,000$ sub-arrays. Para cada sub-array, calcule el número de líneas de base y la longitud máxima de línea de base asumiendo que las 20 antenas están ubicadas en posiciones aleatorias dentro de un círculo de radio 5 km. Genere un histograma de la distribución de la línea de base máxima utilizando seaborn, con un kernel density estimate superpuesto.
\item Implemente un algoritmo de fuerza bruta en Python que, para un caso reducido de $n = 10$ antenas y $k = 4$, genere todas las combinaciones posibles y filtre aquellas que cumplen la restricción del inciso c (a lo sumo 2 antenas de un grupo prohibido de 3). Compare el conteo resultante con el cálculo analítico adaptado a estos nuevos valores e imprima ambos resultados en consola.
\end{enumerate}
