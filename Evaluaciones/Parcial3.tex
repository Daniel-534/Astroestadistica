...
\begin{titlepage}
  ...
  \includegraphics[width=0.24\textwidth]{logo-udea.png}\\[0.7cm]

  {\Large \textbf{Universidad de Antioquia}}\\[0.25cm]
  {\large Facultad de Ciencias Exactas y Naturales}\\[0.15cm]
  {\large Instituto de Física}\\[0.15cm]
  {\large Curso: Astroestadística}\\[1.0cm]

  \rule{\linewidth}{1pt}\\[0.75cm]

  {\LARGE \bfseries Solución del problema práctico\\[0.35cm]
  Parcial 3 de Astroestadística}\\[0.85cm]

  {\large \itshape Modelo estadístico bayesiano y comparación bootstrap\\[0.15cm]
  para la relación período--luminosidad de estrellas Cefeidas}\\[0.75cm]

  \rule{\linewidth}{1pt}\\[1.0cm]

  {\large \textbf{Autor:} Daniel Soto Villada}\\[0.5cm]
  {\large \textbf{Fecha:} \today}

  \vfill
  {\small Medellín, Colombia}\\[0.7cm]
\end{titlepage}
\clearpage
...
\begin{tikzpicture}[
  ...
  \node[font=\scriptsize, text=black!65, align=center] at (5.2,-3.45)
    {Selección predictiva entre modelo lineal y cuadrático mediante remuestreo};
\end{tikzpicture}
...
\section{Material complementario y cálculos reproducibles}

Los cálculos completos implementados en Python, incluyendo la lectura de datos,
los estimadores MAP, las simulaciones posteriores, el bootstrap y las pruebas de
comparación, se encuentran disponibles en el notebook de Google Colab anexado en
la bibliografía; al hacer clic sobre la referencia se abre directamente el enlace
correspondiente \cite{colab_parcial3_solucion}.

La solución de este parcial se desarrolló principalmente siguiendo las
explicaciones de la clase del 21 de mayo de 2026 \cite{clase_20260521} y
estudiando la teoría práctica del documento sobre jackknife, bootstrap y la
prueba de Kolmogorov--Smirnov \cite{jackknife_bootstrap_ks}.
