...
\input{Figuras2/arbol_espirales.tex}
...
\input{Figuras2/esfera_flujo_estelar.tex}
...
\input{Figuras2/densidad_distancia_estelar.tex}
...
\input{Figuras2/densidad_flujo_estelar.tex}
\end{enumerate}
...
$$
\lambda=\frac{4}{100\,\mathrm{pc}}=0.04\,\mathrm{pc}^{-1}.
$$
En un proceso homogéneo de Poisson, el número de estrellas en un segmento de longitud $L$ tiene distribución
$$
N(L)\sim\operatorname{Poisson}(\mu_L),
\qquad \mu_L=\lambda L.
$$
La tasa $\lambda$ se mide por unidad de longitud, mientras que $\mu_L$ es el número esperado de estrellas en el segmento completo. Su función de probabilidad es
$$
P\bigl(N(L)=k\bigr)=\frac{(\lambda L)^k}{k!}e^{-\lambda L},
\qquad k=0,1,2,\ldots
$$
...
$$
\mu_{50\,\mathrm{pc}}=(0.04\,\mathrm{pc}^{-1})(50\,\mathrm{pc})=2.
$$
...
\input{Figuras2/poisson_conteo.tex}

\item Para una longitud general $L$, la probabilidad de encontrar al menos una estrella es
$$
P\bigl(N(L)\ge1\bigr)=1-e^{-\lambda L}.
$$
Exigimos que esta probabilidad sea mayor o igual que $0.90$:
\begin{align*}
1-e^{-\lambda L}\ge0.90
&\quad\Longleftrightarrow\quad e^{-\lambda L}\le0.10\\
&\quad\Longleftrightarrow\quad -\lambda L\le\ln(0.10)\\
&\quad\Longleftrightarrow\quad L\ge\frac{-\ln(0.10)}{\lambda}
=\frac{\ln 10}{\lambda}.
\end{align*}
Al dividir por $-\lambda<0$, se invierte el sentido de la desigualdad. Además, $1-e^{-\lambda L}$ es estrictamente creciente en $L$, pues su derivada es $\lambda e^{-\lambda L}>0$. Por ello, la longitud mínima se obtiene cuando se cumple la igualdad:
$$
...
\input{Figuras2/poisson_longitud.tex}
