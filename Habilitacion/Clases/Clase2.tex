...
Si $A$ y $B$ son dos eventos tales que $P(B) > 0$, la probabilidad condicional de $A$ dado que $B$ ha ocurrido se define como (Devore, Capítulo 2, Sección 2.4):
...
\begin{itemize}
    \item $V$: El evento astrofísico subyacente es \textit{verdadero} y genuino (ej. una kilonova o supernova real).
...
Dos eventos $A$ y $B$ se consideran independientes si la ocurrencia de uno de ellos no proporciona ninguna información sobre la ocurrencia del otro (Devore, Capítulo 2, Sección 2.5). 
...
Es crucial destacar una distinción analítica fundamental (Devore, Sección 2.5): la independencia y la exclusión mutua son conceptos radicalmente distintos. Si dos eventos son mutuamente excluyentes ($A \cap B = \emptyset$), saber que $A$ ocurrió garantiza que $B$ no ocurrió ($P(B|A) = 0$), lo cual implica una dependencia extrema. Por el contrario, si son independientes, la ocurrencia de $A$ no altera en absoluto la probabilidad de $B$.
