...
En la astronomía de grandes sondeos temporales (como los realizados por el Observatorio Vera C. Rubin), los "pipelines" de reducción de datos deben filtrar miles de alertas nocturnas. Analicemos la detección de transitorios.
...
\section{Regla de probabilidad total}

La regla de probabilidad total permite calcular la probabilidad de un evento $B$ cuando este puede ocurrir bajo diferentes escenarios o causas mutuamente excluyentes. Sean $E_1,E_2,\dots,E_n$ eventos que forman una \textit{partición} del espacio muestral $S$; es decir:

\begin{itemize}
    \item Los eventos son mutuamente excluyentes: $E_i\cap E_j=\emptyset$ para $i\neq j$.
    \item Los eventos son exhaustivos: $\displaystyle\bigcup_{i=1}^{n}E_i=S$.
    \item Cada evento tiene probabilidad positiva: $P(E_i)>0$.
\end{itemize}

La deducción comienza usando que $S$ contiene todos los resultados posibles y, por tanto, $B=B\cap S$. Como los eventos $E_i$ cubren todo el espacio muestral, podemos sustituir $S$ por su partición:

\begin{align*}
B
    &= B\cap S\\
    &= B\cap\left(\bigcup_{i=1}^{n}E_i\right)\\
    &= \bigcup_{i=1}^{n}(B\cap E_i).
\end{align*}

Las intersecciones $B\cap E_i$ son mutuamente excluyentes, porque cada una está contenida en un elemento diferente de la partición. Por la aditividad de la probabilidad:

$$ P(B)=P\left(\bigcup_{i=1}^{n}(B\cap E_i)\right)
=\sum_{i=1}^{n}P(B\cap E_i). $$

Finalmente, aplicamos la regla de multiplicación a cada término:

$$ P(B\cap E_i)=P(B|E_i)P(E_i). $$

Sustituyendo esta identidad en la suma anterior se obtiene explícitamente la \textbf{regla de probabilidad total}:

$$ \boxed{P(B)=\sum_{i=1}^{n}P(B|E_i)P(E_i)} $$

Esta expresión pondera la probabilidad condicional de $B$ en cada escenario por la frecuencia con la que ocurre dicho escenario.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[
    font=\footnotesize,
    partition/.style={draw=black!55, thick},
    regionlabel/.style={align=center, font=\scriptsize\bfseries},
    intersection/.style={align=center, font=\scriptsize, text=red!70!black}
]

\fill[blue!7] (-5,-2.1) rectangle (-3,2.1);
\fill[gray!10] (-3,-2.1) rectangle (-1,2.1);
\fill[blue!4] (-1,-2.1) rectangle (1,2.1);
\fill[gray!10] (1,-2.1) rectangle (3,2.1);
\fill[blue!7] (3,-2.1) rectangle (5,2.1);
\draw[partition, rounded corners=4pt] (-5,-2.1) rectangle (5,2.1);
\foreach \x in {-3,-1,1,3}
    \draw[partition] (\x,-2.1) -- (\x,2.1);

\node[anchor=north west, font=\scriptsize\bfseries] at (-4.82,1.92) {$S$};
\node[regionlabel, text=blue!70!black] at (-4,1.48) {$E_1$};
\node[regionlabel, text=black!65] at (-2,1.48) {$E_2$};
\node[regionlabel, text=blue!70!black] at (0,1.48) {$E_3$};
\node[regionlabel, text=black!65] at (2,1.48) {$\cdots$};
\node[regionlabel, text=blue!70!black] at (4,1.48) {$E_n$};

\draw[red!70!black, fill=red!10, very thick]
    (0,-0.20) ellipse [x radius=4.35cm, y radius=1.05cm];
\node[fill=white, inner sep=1pt, text=red!75!black, font=\scriptsize\bfseries]
    at (0,1.02) {evento $B$};

\node[intersection] at (-4,-0.20) {$B\cap E_1$};
\node[intersection] at (-2,-0.20) {$B\cap E_2$};
\node[intersection] at (0,-0.20) {$B\cap E_3$};
\node[intersection] at (2,-0.20) {$\cdots$};
\node[intersection] at (4,-0.20) {$B\cap E_n$};

\node[align=center, font=\scriptsize\bfseries] at (0,-2.65)
    {$B=\displaystyle\bigcup_{i=1}^{n}(B\cap E_i)$\qquad
     $P(B)=\displaystyle\sum_{i=1}^{n}P(B|E_i)P(E_i)$};

\end{tikzpicture}
\caption{Interpretación general de la regla de probabilidad total. El rectángulo representa el espacio muestral $S$, dividido por la partición $\{E_1,E_2,\dots,E_n\}$. El evento $B$ atraviesa varios elementos de la partición y se descompone en las regiones disjuntas $B\cap E_i$, cuyas probabilidades se suman para obtener $P(B)$.}
\end{figure}

\subsection{Ejemplo: Calidad de imágenes bajo diferentes condiciones atmosféricas}

Durante una noche de observación, las condiciones atmosféricas se clasifican en tres estados mutuamente excluyentes:

\begin{itemize}
    \item $A_1$: Cielo despejado, con $P(A_1)=0.50$.
    \item $A_2$: Presencia de nubes delgadas, con $P(A_2)=0.30$.
    \item $A_3$: Condiciones pobres, con $P(A_3)=0.20$.
\end{itemize}

Sea $B$ el evento \textit{obtener una imagen científicamente utilizable}. La experiencia del observatorio indica que:

$$ P(B|A_1)=0.95,\qquad P(B|A_2)=0.60,\qquad P(B|A_3)=0.10 $$

Aplicando la regla de probabilidad total:

\begin{align*}
P(B) &= P(B|A_1)P(A_1)+P(B|A_2)P(A_2)+P(B|A_3)P(A_3)\\
     &= (0.95)(0.50)+(0.60)(0.30)+(0.10)(0.20)\\
     &= 0.475+0.180+0.020=0.675.
\end{align*}

Por lo tanto, antes de conocer las condiciones atmosféricas de una noche particular, la probabilidad global de obtener una imagen utilizable es del $67.5\%$. Esta probabilidad combina todos los escenarios posibles y constituye el tipo de término de normalización que aparece posteriormente en el Teorema de Bayes.
